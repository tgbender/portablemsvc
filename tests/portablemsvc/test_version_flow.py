...
def test_normal_install_db_has_correct_version_fields(normal_test_install):
    ...
    assert "." in toolset_msvc, f"msvc_toolset_version should be format like '14.44': {toolset_msvc}"
...
def test_get_path_finds_by_lockfile(normal_test_install, lockfile_test_install, portablemsvc_exe):
    ...
    for name, install in [
        ...
        try:
            ...
            assert Path(found_path).exists(), f"{name}: get-path returned non-existent path: {found_path}"
...
def test_version_consistency_between_installs(normal_test_install, lockfile_test_install):
    ...
    for name, install in [
        ...
        assert all(p.isdigit() for p in parts), f"{name}: msvc_toolset_version parts should be numeric"
        ...
        assert sdk_parts[2] == sdk_build, f"{name}: sdk_version part 3 should match build_number: {sdk_parts[2]} vs {sdk_build}"
